...
function initMap() {
    ...
    map.on('zoomend', function() {
        renderTrees(); // Re-render on zoom to handle dynamic sizes or icons
    });
}

// Load Metadata index and initialize dropdown
async function loadInitialMetadata() {
    loader.classList.remove('hidden');
    loaderText.textContent = "Cargando catálogo de especies...";
    
    try {
        // 1. Cargar especies únicas
        const speciesResponse = await fetch('data/species.json?v=' + new Date().getTime());
        if (!speciesResponse.ok) throw new Error("Error loading species metadata.");
        speciesList = await speciesResponse.json();

        // 2. Cargar metadatos de distritos
        loaderText.textContent = "Cargando distritos...";
        const districtsResponse = await fetch('data/districts.json?v=' + new Date().getTime());
        if (!districtsResponse.ok) throw new Error("Error loading districts metadata.");
        districtsMetadata = await districtsResponse.json();

        // 3. Poblar selector de distritos
        districtFilter.innerHTML = '';
        districtsMetadata.forEach(d => {
            // Excluir "Desconocido" si no tiene casi datos
            if (d.count > 10) {
                const option = document.createElement('option');
                option.value = d.filename;
                option.textContent = `${d.name} (${d.count.toLocaleString()} árboles)`;
                districtFilter.appendChild(option);
            }
        });

        // 4. Cargar el primer distrito por defecto (ej. Casco Antiguo)
        const defaultDistrict = districtsMetadata.find(d => d.name === "Casco Antiguo") || districtsMetadata[0];
        districtFilter.value = defaultDistrict.filename;
        
        await loadDistrictData(defaultDistrict.filename);

    } catch (error) {
        console.error(error);
        alert("Error cargando metadatos iniciales: " + error.message);
    } finally {
        loader.classList.add('hidden');
    }
}

// Load dynamic district JSON data on demand
async function loadDistrictData(filename) {
    loader.classList.remove('hidden');
    loaderText.textContent = "Descargando datos del distrito...";
    
    try {
        const response = await fetch(`data/${filename}?v=` + new Date().getTime());
        if (!response.ok) throw new Error("No se pudieron cargar los datos de este distrito.");
        
        allTrees = await response.json();
        
        // Enfocar el mapa al centro geográfico del distrito cargado
        if (allTrees.length > 0) {
            ...
            if (count > 0) {
                map.setView([sumLat / count, sumLon / count], 14.5);
            }
        }
        ...
        console.error(error);
        alert("Error al cargar el distrito: " + error.message);
    } finally {
        loader.classList.add('hidden');
    }
}
...
function renderTrees() {
    ...
    // Aquí filtramos sobre allTrees, que ya contiene EXCLUSIVAMENTE los árboles del distrito activo
    ...
    filteredTrees.forEach((tree, idx) => {
        ...
        if (showFlowers && tree.flower_color) {
            if (currentZoom < 15 && filteredTrees.length > 5000) {
...
function setupEvents() {
    // Al cambiar de distrito, cargamos dinámicamente su JSON específico
